...
$(document).ready(function () {
  ...
  checkScreenWidth();
  randomizeScoringObjectives();
  setScoringObjectives();
});
...
$(document).keydown(function (e) {
  if (!lockMap) {
    ...
    var rotateOrNatureCubeMode =
      rotateTileAllowed == true || natureCubeMode == true;

    if (e.which == 81 && rotateOrNatureCubeMode) {
      rotateTileCounterClockwiseFunction();
      return false;
    } else if (e.which == 69 && rotateOrNatureCubeMode) {
      rotateTileClockwiseFunction();
      return false;
    } else if (e.which == 82 && rotateOrNatureCubeMode) {
      ...
      return false;
    } else if (e.which == 70 && rotateOrNatureCubeMode) {
      cancelTilePlacement(natureCubeMode ? 'natureCubeTile' : 'normalTile');
      return false;
...
function pickNewTilesTokens() {
  ...
  leftOverTiles = leftOverTiles.sort(function (a, b) {
    return b - a;
  });
  $(
    '.tokenTileContainer[tokentilenum="' +
  ...
  leftOverTokens = leftOverTokens.sort(function (a, b) {
    return b - a;
  });
  $(
    '.tokenTileContainer[tokentilenum="' +
...
$(document).on(touchEvent, '#randomizeButton', function () {
  randomizeScoringObjectives();
  setScoringObjectives();
});
...
function randomizeScoringObjectives() {
  var shuffledBear = shuffle(allScoringObjectives.bear);
  var shuffledElk = shuffle(allScoringObjectives.elk);
  var shuffledFox = shuffle(allScoringObjectives.fox);
  var shuffledHawk = shuffle(allScoringObjectives.hawk);
  var shuffledSalmon = shuffle(allScoringObjectives.salmon);

  scoringObjectives.bear = shuffledBear[0];
  scoringObjectives.elk = shuffledElk[0];
  scoringObjectives.fox = shuffledFox[0];
  scoringObjectives.hawk = shuffledHawk[0];
  scoringObjectives.salmon = shuffledSalmon[0];
}

function setScoringObjectives() {
  $('[id=bear]').text(scoringObjectives.bear);
  $('[id=elk]').text(scoringObjectives.elk);
  $('[id=fox]').text(scoringObjectives.fox);
  $('[id=hawk]').text(scoringObjectives.hawk);
  $('[id=salmon]').text(scoringObjectives.salmon);
}
